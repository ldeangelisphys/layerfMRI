...
nbin <- 8                 # check the file names in the results_dir folder, e.g 10
clusterSizeThr <- 100
...
ISC_estimates <- read.csv(paste0(results_dir,"/",ISC_flavour,"_JU_time_courses_thr",clusterSizeThr,"_bin", nbin, "_isc.csv"),
                stringsAsFactors = F)
```
...
res_descriptives %>%
  ...
  ylab("Mean ISC estimate across participants") + xlab("Cortical Depth bins :  1 = WM/GM border, 6 = pial") +
  theme(
