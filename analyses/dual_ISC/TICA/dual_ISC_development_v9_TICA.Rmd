---
title: "Dual ISC development v9 TICA - a big love furrring nest"
...
clusterSizeThr <- 100   # the max thr to have proper data in the end
                       # if I use 100 I don't have the OCC time courses in sub_11
...
This is subject/taskrun/contrast specific. Note that since now we use ISC binary maps instead of native-space GLM Z maps, the zthr is set to > 0 (in the first cell of the notebook)
...
get_mean_tc <- function(fmri2D, idx_voxels, start_TR, end_TR, ExpectedDuration) {

  zscore <- function(x, na.rm = T) (x - mean(x, na.rm = na.rm)) / sd(x, na.rm)
      
  idx <- unlist(idx_voxels)
  
  # There is a mismatch between the expected and the recorded duration 
  # for some of the intact movies. 
  # This prevents having concatenated time courses of the same length. 
  # Therefore the recorded duration (start_TR : (end_TR-1)) was replaced
  # with the estimated duration.
  # See TICA_preliminary_checks.Rmd for more info
  
  # OLE : recorded duration
  # tcs_at_idx_voxels <- fmri2D[idx, start_TR : (end_TR-1)]
  
  # NEW : estimated duration
  tcs_at_idx_voxels <- fmri2D[idx, (start_TR-1) : ((start_TR-1) + (ExpectedDuration-1))]
...
# 
# mini <- mini %>%
#   mutate(tc_mean = list(get_mean_tc(nii_list$fmri2D, idx_voxels, start_TR, end_TR, ExpectedDuration)) )
# 
# test_df <- test_df %>%
#   rowwise() %>%
#   mutate(tc_mean = list(get_mean_tc(nii_list$fmri2D, idx_voxels, start_TR, end_TR, ExpectedDuration)) )
...
Since the function to calculate the mean tc is `rowwise`, it would in principle require to store the _whole_ fmri2D in each row, which would take an astronomical amount of time (let alone the fact that the memory would exhaust much before that). 
...
DO_TASKRUN <- function(df_taskrun) {
  ...
  unnest(shebang) %>%               # 4. idx_voxels for each (movie/contrast)/JU/bin
  {
    ...
    rowwise(.) %>% 
      ...
      mutate(tc_mean = list(get_mean_tc(fmri$fmri2D, idx_voxels, start_TR, end_TR, ExpectedDuration)) )
  } 
...
# pf %>%
#   group_by(JU) %>%
#   select(muvi, ExpectedDuration, tc_mean) %>%
#   group_split()
...
# # ------------ Some checks --------------------
# 
# # The lenght of the concatenated movies should be 1148 (cfr TICA_preliminary_checks.Rmd)
# # This is correct after using (start_TR-1) : ((start_TR-1) + (ExpectedDuration-1))
# pf %>%
#   group_by(sub,JU,D_bins) %>%
#   select(contrast, sub, ses, task, run, muvi, D_bins, tc_mean) %>%
#   unnest(tc_mean) %>%
#   count() %>%
#   group_by(JU) %>%
#   group_split()
...
## Check-ups before concatenation
```{r}

# pf_homogeneous %>% 
#   filter(sub == "sub_02", ses == "ses_01", task == 1, run == 1) %>%  # select one sub/taskrun
#   group_by(JU, D_bins) %>% 
#   select(contrast, sub, ses, task, run, muvi, D_bins, tc_mean) %>% 
#   group_split()


poppa <- pf_homogeneous %>% 
  # filter(sub == "sub_02", ses == "ses_01", task == 1, run == 1) %>%  # select one sub/taskrun (alternative to the next row)
  filter(sub == "sub_02") %>%  # select all taskrun for one sub
  filter(JU == 13, D_bins == 1) %>% # isolate one bin in one JU
  select(contrast, sub, ses, task, run, muvi, D_bins, tc_mean)


poppa %>% count()
# the result of the pijp above is 72 rows, i.e. 36*2 muvis, CORRECT 


# concatenation can be simply obtained using unnest (amazing, right?)  
poppa %>% 
  unnest(tc_mean) %>% count()

# the result of the pijp above is 1198 rows, which is exactly the 
# sum of all the TRs for all the muvis. CORRECT


# This is the crucial test: all the concatenated time courses in all sub and JU
# MUST have the same length.
# Uncomment the 'summarise' lines to have a quick summary
pf_homogeneous %>% 
  inner_join(julabels, by = c("JU"="numba")) %>%  # give names to the JUs
  mutate(JU = name) %>% 
  group_by(sub,JU,D_bins) %>% 
  select(contrast, sub, ses, task, run, muvi, D_bins, tc_mean) %>% 
  unnest(tc_mean) %>%
  count(name = "TC_length") %>%
  group_by(sub) %>%
  # summarise(
  #   min_TC_length = min(TC_length),
  #   max_TC_length = max(TC_length),
  #   .groups = "drop"
  # )
  group_split()
...
## Concatenation (1) - Prepare tcs dataframe
This step is particularly delicate, since R is not particularly suited for tensors 
(i.e. n-dim arrays). I need to put all the values of tc_mean for one sub, 
each bin and each JU in a single column, which I will treat as a long vector to 
reshape into a 6 (layers) x 3 (JUs) x 1 x nTR (=1148) array.

To do that, the arrangement must be correct:

- first arrange by sub, to be able to split the whole dataframe between subjects
(actually creating a list of df indexed by sub)

- then arrange by JU and D_bins, in this order. Note that I change the ID of 
BA6 to 21 to have it after BA44 (ID=13) and before PFt (ID=35)

- finally, I will pipe the resulting list into a map which will produce a 4D
array for each sub

NB: I checked the correct ordering by visual inspection of tcs
```{r}
# Make sure that there are 72 movies for each sub,JU,bin, and that they are
# ordered in the same way
# Select only the relevant JUs (BA44:13, PFt:35, BA6:91->21)
pf_homogeneous %>% 
  inner_join(julabels, by = c("JU"="numba")) %>%  # give names to the JUs
  mutate(JU_name = name) %>%
  select(sub,muvi,D_bins,tc_mean,JU,JU_name) %>% 
  filter(JU %in% c(13,35,91)) %>%  # select only the relevant JUs
  arrange(sub, muvi) %>%  # should already be arranged, but better safe than sorry
  group_by(sub,JU_name,D_bins,JU) %>%
  count(name="n_muvis") %>%
  group_by(sub) %>% 
  group_split()
...
pf_homogeneous %>% 
  ...
  select(sub,muvi,D_bins,tc_mean,JU,JU_name) %>% 
  filter(JU %in% c(13,35,91)) %>%  # select only the relevant JUs
  mutate(JU = ifelse(JU==91,21,JU)) %>%
  arrange(sub, muvi) %>%  # should already be arranged, but better safe than sorry
  filter(sub == "sub_02") %>% # TESTING ONLY ON ONE SUB
  group_by(sub,D_bins,JU) %>% 
  unnest(tc_mean) %>%
  arrange(sub,JU,D_bins) %>%   # the actual concatenation must be preceded by this ordering
  select(sub,D_bins,JU,tc_mean, JU_name) %>% 
  group_split() # this is what will happen 

  
# creating the tcs to reshape into matrices
tcs <- pf_homogeneous %>% 
  inner_join(julabels, by = c("JU"="numba")) %>%  # give names to the JUs
  mutate(JU_name = name) %>%
  select(sub,muvi,D_bins,tc_mean,JU,JU_name) %>% 
...
# Test the creation of the list of sub
tcs %>% 
  group_split() 
...
# Test the ordering ON ONE SUB ONLY
poppa <- tcs %>% 
  filter(sub == "sub_02") %>% 
  arrange(sub,D_bins,JU) %>%
  mutate(n = 1:20664) %>% 
  select(-tc_mean) %>% 
  group_by(sub,D_bins,JU,JU_name) %>% 
  nest

poppa


# Make sure that the correct values fall in the correct places
# (the following lines are equivalent)
map(c(min,max,length), ~ poppa$data[1] %>% unlist %>% .x) %>% set_names(c("min","max","length"))

poppa$data[18] %>% map2(c(min,max,length), ~  .x %>% unlist %>% .y) %>% set_names(c("min","max","length"))


# try to build the matrix on synthetic data
tot_vals = 20664
voxels = 18

ts_length <- tot_vals / voxels

# 3D array
array(1:tot_vals, dim = c(ts_length,6,3)) %>% aperm(c(2,3,1))

# 4D array, adding a singleton
arr <- array(1:tot_vals, dim = c(ts_length,6,3,1)) %>% aperm(c(2,3,4,1))
...
getmat <- function (df) {
  df %>% 
    select(tc_mean) %>% 
    pull %>% 
    array(dim = c(ts_length,6,3,1)) %>% aperm(c(2,3,4,1))
}
...
## Do some checks before writing the NIFTIs
```{r}


testmat <- function(sub,JU,bin) {
  
  listsub <- unique(tcs$sub) %>% as.list
  listJU_tcs = list("BA44" = 13, "BA6" = 21, "PFt" = 35)

  indf <- tcs %>% 
    filter(sub == !!listsub[sub], D_bins == bin, JU == !!listJU_tcs[JU]) %>%
    ungroup %>% 
    select(tc_mean) %>% pull
  
  inmat <- mats[[sub]][bin,JU,1,]
  ...
  par(mfrow=c(2,2))
  plot(indf,inmat,type='l', main = titolo)
  plot(indf,type = 'l',main = "indf")
  plot(inmat,type = 'l',main = "inmat")
  plot(inmat-indf,type = 'l',main = "inmat-indf" )
  
}


testmat(sub=1, JU=1, bin=1)

```


## Separate Motion from Scrambled and write the niftis
```{r}

nii <- asNifti(mats[[1]])
writeNifti(nii, paste0(TICAdir,"/subtest.nii.gz"))


# Intacts are on top, and the number of time points is exactly the half (as expected)
idx_Motion <- 1:(1148/2)
idx_Scrambled <- (1148/2 + 1):1148

mats_Motion <- mats %>% map(~ .x[,,1,idx_Motion, drop = F])
mats_Scrambled <- mats %>% map(~ .x[,,1,idx_Scrambled, drop = F])


# Write niftis
listsub <- unique(pf_homogeneous$sub)

library(RNifti)

writeimgs <- function(sub,mat,flavour) {
  nii <- asNifti(mat)
  writeNifti(nii, paste0(TICAdir,"/nii4TICA/",sub,"_",flavour,".nii.gz"))
}

walk2(listsub, mats_Motion, ~ writeimgs(.x, .y, "M"))
walk2(listsub, mats_Scrambled, ~ writeimgs(.x, .y, "S"))
...
filename_RDS_2_write <- paste0(ISC_flavour,"_JU_time_courses_thr",clusterSizeThr,"_bin",nbin,".rds")

saveRDS(pf_homogeneous, file = paste0(TICAdir,filename_RDS_2_write))
