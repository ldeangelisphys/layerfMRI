...
clusterSizeThr <- 100  # to build a meaningful histogram
nbin <- 20 # set hist breaks to a fixed range
thr_N_taskruns <- 1 # in how many taskruns (>2) should I find sig clusters, to consider it interesting?
...
all <- map(
  ...
  function(onesub) map(
    contrast_list, 
    function(contrast) get_results(
      SUBID = onesub, 
      Zcontrast = contrast, 
      zthr, clusterSizeThr, nbin, thr_N_taskruns
    )
  )
)
