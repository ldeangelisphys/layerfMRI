...
**Quantifying the proportion of suprathreshold voxels across layers instead of Z value**\
...
```
rmarkdown::render("depth_native_analysis_V[version_numba].Rmd", 'pdf_document')
```
...
**NB**: the two functions are separate so that I don't need to reload the niis if I choose another Z threshold
...
## Get mean Z, their count for each D bin in each JU region in each taskrun, as well as their proportion across cortical depth layers
Here we use the dataframe `vals` created above - containing each subtibbles, one for each taskrun - to sample the depth in `nbin` bins (e.g. deciles if `nbin` = 10) and then extract the mean suprathreshold Z value for each cortical depth bin. Importantly, we get also the count of suprathreshold voxels and their proportion in each bin wrt to every other bin. This calculation is carried out separately for each taskrun (using `map`), and for each ROI.
...
7. calculate mean Z in each D bin
8. calculate the proportion of activated voxels in each bin wrt to all other bins. **This is our desired metric of interest**. The final tibble of each ROI has N rows - one for each bin - and two columns for prop_Z and JU name.
9. bind all the ROI-specific tibbles by rows. Therefore in the final tibble there will be N JU_ROIs * 10 rows
...
As a matter of fact, the main group-level results do not change much using either function.
...
get_meanZ <- function(vals, clusterSizeThr, nbin) {
  ...
  ALL_taskruns_meanZ <- vals %>% 
    ...
    map(~ .x %>% mutate(prop_Z = numba_Z/sum(numba_Z)) ) %>%  # 8. create a column with the proportion 
    bind_rows()                         # 9. put the summary of all the JU ROIs
...
# ALL_taskruns_meanZ <- map(list(vals[[1]]), ~ get_meanZ(.x, clusterSizeThr, nbin))
# 
...
## Average the proportion of suprathreshold Z voxels (across D bins) across taskruns

1. Before averaging, include only ROIs where sig clusters where found in an acceptable numba of runs (thr_N_taskruns)
2. Split the tibble in N tibbles, where N is the numba of JU ROI with sig clusters in at least n_aboveThr_taskrun
3. For each ROI (map) group the prop_Z values by bin, so that you will be able to estimate the average prop_Z for each Depth bin across taskruns
4. Calculate the mean of prop_Z across taskruns. **This is the quantity that will be taken to the group-level analysis**
...
get_MEAN_taskruns_meanZ <- function(ALL_taskruns_meanZ, thr_N_taskruns) {
  ...
  MEAN_taskruns_meanZ <- ALL_taskruns_meanZ %>% 
    ...
    map(~ .x %>% summarise(                # 4. get the mean_Z for each D
      TR_mean_Z = mean(prop_Z),    # numba_Z = count; prop_Z = proportion - in case you want to try
      # sd_Z = sd(mean_Z),
      JU = mean(JU),
      .groups = "drop"
    )) %>% bind_rows()
...
thr_N_taskruns <- 2    # in how many runs there should be sig clusters, to be interesting?
...
For each subject, run and condition (Motion vs. Scrambled) we considered only those voxels whose Z value survived whole-brain correction (using GRF p=0.05 on an initial Z map thresholded at `r zthr`) in each of the two contrasts: `r names(contrast_list)[1]` and `r names(contrast_list)[2]`.

We then quantified the proportion of suprathreshold voxels in each cortical depth bin with respect to all the `r nbin` bins in which the cortical depth was sampled. This was repeated for each region of the cytoarchitectonic atlas of Juelich transported into the native space of each fMRI run (8 for each participant).

To yield meaningful estimates, we considered only regions with at least `r clusterSizeThr` suprathreshold voxels. Note that since this value was calculated on already corrected Z maps (see above), the surviving voxels are likely highly clustered, rather than scattered across the extent of the region of interest.

The final parameter estimate of brain activity in either contrast for each cortical depth bin consisted of the mean proportion of suprathreshold voxels across runs. For the group-level analysis, we considered only Juelich regions featuring at least `r clusterSizeThr` significant voxels in at least `r thr_N_taskruns`/8 runs at the subject level. Finally, we compared our estimate between constrasts only in regions where the above conditions were met in at least `r thr_N_subs` participants.
...
| -------------  | ------------------ | --------------------------------------------------- |
| zthr           | `r zthr`           | whole-brain corrected Z thr in each run             |
| clusterSizeThr | `r clusterSizeThr` | min numba voxels in each JU ROI                     |
| nbin           | `r nbin`           | numba of Depth bins                                 |
| thr_N_taskruns | `r thr_N_taskruns` | thr for min numba of runs with sig amount of voxels |
| thr_N_subs     | `r thr_N_subs`     | thr for min numba subs in which the effect was sig  |
...
res <- res %>% 
  ...
  mutate(pcorr_sig = ifelse(p <= 0.05, D_bins, NA))  # to print asterisk on sig p bins
...
res %>%
  ...
  kbl(caption = "Significant differences uncorrected and corrected ( $q(FDR) = 0.05$ ) in Motion > Scrambled" ) %>% 
...
res %>% 
  ...
  labs(
    ...
    subtitle = "Comparison of the proportion of suprathreshold voxels in each bin in either contrast"
  ) +
...
res_descriptives %>%
  ...
  theme_minimal() +
  labs(
    title = "Mean proportion of suprathreshold voxels for each Depth bin in each Juelich region across participants"
  ) +
  ylab("Mean proportion of active voxels across the whole cortical depth") + xlab("Cortical Depth bins :  1 = WM/GM border, 6 = pial") +
  theme(
    strip.text.x = element_text(size = 12), # increase size of title for each subplot
    panel.spacing.y = unit(3, "lines")      # increase space between facets
  )  
