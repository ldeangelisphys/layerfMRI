...
# 3. create a column of *proportions* (prop_Z) instead of simple count (numba_Z)
ALL_taskruns_meanZ <- ALL_taskruns_meanZ %>% 
  group_by(JU,taskrun) %>% 
  mutate(prop_Z = numba_Z/sum(numba_Z)*100)
...
get_res_contrast <- function(SUBID, Zcontrast, zthr, clusterSizeThr, nbin, thr_N_taskruns) {
  ...
  ALL_taskruns_meanZ <- ALL_taskruns_meanZ %>% 
    group_by(JU,taskrun) %>% 
    mutate(prop_Z = numba_Z/sum(numba_Z) )
  
  # 4. Average the mean Z in each D bin across taskruns
  MEAN_taskruns_meanZ <- get_MEAN_taskruns_meanZ(ALL_taskruns_meanZ, thr_N_taskruns)
...
thr_N_taskruns <- 2    # in how many runs there should be sig clusters, for this sub to be interesting?
...
### Permutation t-test
```{r compare-between-contrasts-perm}
...
# 
# 
# # Permutation t-test by Karl Broman
# library("broman")
# 
# compare_contrasts_perm <- function(df) {
#   df_diff <- df$Motion - df$Scrambled
#   paired.perm.test(df_diff, n.perm = 5000, pval = TRUE)
# }
# 
# 
# # Carry out the comparison Motion > Scrambled for each D_bin in each JU ROI
# ttest_res <- all_nest %>% 
#   mutate(
#     p_perm = map_dbl(data, compare_contrasts_perm )
#   ) %>% 
#   mutate(
#     ttest = map(data, compare_contrasts )
#   )
...
res %>%
  ...
  kbl(caption = "Significant differences either uncorrected or corrected ( $q(FDR) = 0.05$ ) in Motion > Scrambled" ) %>% 
