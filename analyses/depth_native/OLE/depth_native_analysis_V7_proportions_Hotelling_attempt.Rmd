...
get_MEAN_taskruns_meanZ <- function(ALL_taskruns_meanZ, thr_N_taskruns) {
  ...
  MEAN_taskruns_meanZ <- ALL_taskruns_meanZ %>% 
    ...
    map(~ .x %>% summarise(                # 4. get the mean_Z for each D
      TR_mean_Z = mean(prop_Z),    # numba_Z = count; prop_Z = proportion
      # sd_Z = sd(mean_Z),
      JU = mean(JU),
      .groups = "drop"
    )) %>% bind_rows()
...
## Group-level analysis (univariate)
...
## Group-level analysis (multivariate) (1) Preparation
```{r}
...
clusterSizeThr <- 50   # to build a meaningful histogram
nbin <- 10             # bins to sample the cortical Depth
thr_N_taskruns <- 3    # in how many runs there are sig clusters, to be interesting?
thr_N_subs <- 7        # numba subs in which the effect must be sig 
...
# Run the estimation for each subs and put the results in a list
# of tibbles where every tibble is one sub
all <- listsub %>% 
  map( function(sub) get_joined_res(contrast_list, 
                        sub, 
                        Zcontrast, 
...
# Create a column defining to which sub the results belong to and bind_rows
all <- list(all,listsub) %>% pmap(~ .x %>% mutate(sub = .y)) %>% bind_rows()


# Remove ROIs with sig results in less than thr_N_subs
all <- all %>% 
  drop_na() %>%    # remove subs with non-sig results for either contrast
  group_by(JU) %>% 
  mutate(n_sub_sig = n_distinct(sub)) %>% 
  filter(n_sub_sig >= thr_N_subs)
  


# Reshape data to prepare for Hotelling T2 and nest
all <- all %>% 
  group_by(JU) %>%  # 1. reshape the data in each JU independently
  pivot_longer(c("Motion","Scrambled"), names_to = "contrast") %>%  # 2. contrast wide to long
  pivot_wider(names_from = D_bins, names_prefix = "Dbin") %>%  # 3. D_bins long to wide
  arrange(contrast) %>% 
  tidyr::nest() %>% 
  inner_join(., julabels, by = c("JU"="numba")) # attach juelich labels
...
## Group-level analysis (multivariate) (2) Hotelling T2
```{r}

library(ICSNP)

# Using two-sample HotT2 test
compare_contrasts_two_samples <- function(df) {
  
  Motion <- df %>% filter(contrast == "Motion") %>% select(-c(sub,n_sub_sig,contrast))
  Scrambled <- df %>% filter(contrast == "Scrambled") %>% select(-c(sub,n_sub_sig,contrast))

  HotellingsT2(Motion, Scrambled, test = "f")
}


# Using paired-samples HotT2 test
compare_contrasts_paired <- function(df) {
  
  Motion <- df %>% filter(contrast == "Motion") %>% select(-c(sub,n_sub_sig,contrast))
  Scrambled <- df %>% filter(contrast == "Scrambled") %>% select(-c(sub,n_sub_sig,contrast))
  df_diff <- Motion - Scrambled

  # # PCA version
  # df_diff_pc <- prcomp(df_diff, scale. = TRUE)
  # HotellingsT2(df_diff_pc$rotation[, 1:3], test = "f")
  
  # # Regular version
  HotellingsT2(df_diff, test = "f")
}
...
res_HotellingT2 <- all %>% 
  mutate(
    HotT2 = map(data, compare_contrasts_two_samples )
  )
...
# Extract the parameters of interest (T,p)
res_HotellingT2 <- res_HotellingT2 %>% 
  mutate(
    glance = HotT2 %>% map(glance),
    T2 = glance %>% map_dbl("statistic"),
    p = glance %>% map_dbl("p.value")
  )


res_HotellingT2 %>% select(JU, name, T2, p)
...
get_diff <- function(df) {
  ...
  paste0("det(cov) = ", det(cov(df_diff)) ) %>% print()
  mvnormtest::mshapiro.test(t(a)) %>% print()
  
}


all$data[[1]] %>% get_diff() %>% map_dbl(mean) %>% barplot(horiz = TRUE)
