...
### (1) Defun to get the results for each contrast

Returns a dataframe with columns for `[D_bin, TR_mean_Z, JU]` for one subject and **one contrast** (see the result of the previous cell for an example).
...
poppa <- get_res_contrast(SUBID, Zcontrast, zthr, clusterSizeThr, nbin, thr_N_taskruns)

```


### (2) Defun to get the result for both contrasts and join them in one dataframe
...
Now the dataframe is ready for group-level analysis
...
Consider only ROIs where a sig result was found in all 9 sub
...
## Run for all subjects and all contrasts (OLE)
...
```{r group-analysis, eval=F, include=F}
...
```{r, final-plot, fig.width=10, fig.height=10, eval=F, include=F}
