...
# It's outside so that it can be easily modified to be something
# other than t.test
...
The cortical depth was sampled using `r nbin` bins. For each bin, we extracted at each voxel the Z statistics derived from the single-subject GLM carried out in the native space for each of the two contrasts `r names(contrast_list)[1]` and `r names(contrast_list)[2]`. We considered only voxels at Z >= `r zthr` (corrected with GRF using a cluster p threshold of 0.05) with a minimum cluster size of `r clusterSizeThr` voxels and averaged their Z value at each voxel across runs (8 for each participant). Finally, we compared these average Z values across participant for each region of the Juelich atlas in which significant clusters of voxels were present in at least `r thr_N_taskruns` at the participant level.
...
# Plot results
ttest_res %>% 
  select(D_bins, JU, T, p) %>% 
  inner_join(., julabels, by = c("JU"="numba")) %>% 
  mutate(name = str_replace_all(name, "_", " ")) %>% 
  ggplot( aes(x = D_bins, y = T, fill = (1-p)) ) +
  scale_fill_viridis_c(10) +
  geom_bar(stat = "identity", width = 1, position = position_identity()) +
